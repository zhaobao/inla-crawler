...
import (
	...
	"inla/inla-crawler/tasks/music/netease/us/dao"
	"io/ioutil"
	"log"
	"net/http"
	"os"
...
func main() {
	database.Connect(filepath.Join(rootDir, "db.sqlite"))
	//parse()
	serve()
	//createPrimaryColor()
}

const rootDir = `tasks/music/netease/us`
const srcDir = `/Volumes/extend/crawler/music/163/us/Billboard`

func createPrimaryColor() {
	_ = filepath.Walk(filepath.Join(rootDir, "output"), func(path string, info os.FileInfo, err error) error {
		...
		if err == nil {
			if strings.Index(output, "srgb") >= 0 {
				...
				color := parseRgba(ret)
				resId := filepath.Base(filepath.Dir(path))
				log.Println(resId, color)
				_, _ = database.GetInstance().Exec(`update music_163_us set primary_color = ? where res_id = ?`, color, resId)
			}
		}
		return nil
	})
...
func parse() {
	...
	for _, music := range items {
		...
		id, err := dbLink.Exec(`insert into music_163_us(res_id, title, sub_title, cover_link, res_link, 
